...
// Il reste en .ts, pas en .tsx, car il ne retourne aucun JSX.

// 'use server' : directive obligatoire pour indiquer que les fonctions exportees
// dans ce fichier s'executent cote serveur.
...
// Une Server Action est un point d'entree public : il faut valider les donnees
// avant de toucher a Prisma, Stripe ou une variable sensible.
...
// Fonction asynchrone exportee — c'est la Server Action appelee depuis le client
...
export async function createCheckoutSession(
  ...
  if (!parsed.success) {
    ...
    // En Zod 4, la liste des erreurs s'appelle "issues" et non "errors".
    throw new Error(parsed.error.issues[0]?.message ?? 'Entree invalide');
  }
  ...
  const cart = await prisma.cart.findUnique({
    ...
    // include : charger aussi les relations necessaires.
    // Sans include, cart.items ne serait pas disponible.
    include: {
      items: {             // Charger les articles du panier
        include: {
          product: true,   // Pour chaque article, charger aussi le produit associe
        },
      },
    },
  });
  ...
  // Stripe ne comprend pas directement notre modele Prisma, il faut convertir.
  const lineItems = cart.items.map((item) => {
    ...
    // Retourner l'objet au format Stripe line_item
    return {
      price_data: {
  ...
  const checkoutSession = await stripe.checkout.sessions.create({
    ...
    // {CHECKOUT_SESSION_ID} est un placeholder remplace automatiquement par Stripe.
